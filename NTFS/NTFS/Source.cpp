...
string toString(BYTE* sector, int offset, int length)
{
...
string toBinary(int n)
{
    ...
    {
        tam = n % 2;
        SoNhiPhan = SoNhiPhan + (tam * tam1);
        n = n / 2;
        tam1 = tam1 * 10;
    }
    string result = to_string(SoNhiPhan);
    return result;
}
...
void Print_Directory_Tree(int a, int tab, int vt)
{
    tab++;
    print_Tab(tab);
    ...
    for (int i = 0; i < child.size(); i++)
        Print_Directory_Tree(child[i], tab, VT[i]);
}
...
void Read_BPB(BYTE* sector, LPCWSTR disk)
{
    ...
    cout << "|________________________________|________________________" << "|" << endl;
    cout << endl;
...
void Read_Tree(BYTE* sector, LPCWSTR disk)
{
    unsigned int bytes_per_sector = Get_Bytes(sector, 0x0B, 2); // Bytes Per Sector
    unsigned int sectors_per_cluster = Get_Bytes(sector, 0x0D, 1); // Sectors Per Cluster
    unsigned int sectors_per_track = Get_Bytes(sector, 0x18, 2); // Sectors Per Track
    ...
    for (int i = 2; i < len_MFT - MFTStart; i += 2)
    {
        ...
        if (toString(currentEntry, 0x00, 4) == "FILE")
        {
            ...
            if (ID > 38)
            {
                ...
                {
                    int length = Get_Bytes(currentEntry, startData + 4, 4);
                    startData += length;
